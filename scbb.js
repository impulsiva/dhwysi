	...
	var bbcodes = {
		...
		img: {
			...
			format: function ($element, content) {
				...
				return '[simage]' + $element.attr('src') + '[/simage]';
			},
			html: function (token, attrs, content) {
		...
		url: {
			...
			format: function (element, content) {
				...
				if (url.substr(0, 7) === 'mailto:') {
					return '[link=' + url.substr(7) + ']' +
						content + '[/link]';
				}

				return '[link=' + url + ']' + content + '[/link]';
			},
			html: function (token, attrs, content) {
				attrs.defaultattr =
		...
		quote: {
			...
			format: function (element, content) {
				...
				return '[quote]Orijinalden alÄ±ntÄ±:  '+author+'\n\n' + content + '[/quote]';
			},
			html: function (token, attrs, content) {
				if (attrs.defaultattr) {
		...
		youtube: {
			...
			format: function (element, content) {
				...
				return element ? '[DHTUBE=' + element + ']' : content;
			},
